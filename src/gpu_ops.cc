...
// dtype, but we also include one extra method "build_kepler_descriptor" to
// generate an opaque representation of the problem size that will be passed to
...
PYBIND11_MODULE(gpu_ops, m) {
  ...
  m.def("build_kepler_descriptor", [](std::int64_t size) {
